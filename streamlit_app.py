...
def read_ne_airport():
    ...
    df["distance_to_boston"] = df.apply(
        lambda row: haversine(
            BOSTON_COORDS, (row["latitude_deg"], row["longitude_deg"])
        ),
        axis=1,
    )
...
def calculate_dynamic_score(distance, min_distance, max_distance):
    if max_distance == min_distance:  # Avoid division by zero
        return 10
    normalized_score = (
        10 - ((distance - min_distance) / (max_distance - min_distance)) * 9
    )
...
def plot_neighborhood_pie_chart(df, title="Airbnb Neighborhood Distribution"):
    ...
    if "neighbourhood" not in df.columns:
        st.warning(
            "The 'neighbourhood' column is missing in the dataset. Unable to plot pie chart."
        )
        return
    ...
    fig, ax = plt.subplots(
        figsize=(7, 7)
    )  # Slightly larger figure for better visibility
    wedges, texts, autotexts = ax.pie(
        ...
        colors=custom_colors[
            : len(grouped_counts)
        ],  # Limit colors to the number of wedges
...
def main():
    ...
    st.markdown(
        "Find Airbnbs with the best convenience scores near selected New England airports."
    )
    ...
    selected_airport = st.selectbox(
        "Select an airport:",
        airports.index,
        format_func=lambda x: airports.loc[x]["name"],
    )
